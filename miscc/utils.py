...
def save_pickle(data, save_path):
    with open(save_path, 'wb') as f:
        ...
        print('Save to: ', save_path,', congrats!')

def parse_str(ids):
    """
    :param ids: the GPU ids
    :return: ids_list
    """
    str_ids = ids.split(',')
    ids_list = []
...
def adjust_learning_rate(learning_rate, lr_update, optimizer, epoch):
...
def build_super_images2(real_imgs, captions, cap_lens, ixtoword,
    ...
    batch_size = real_imgs.size(0)
    max_word_num = np.max(cap_lens)
    # print('2.max_word_num:', max_word_num)\
    #     ('1.batch_size:', 1)
    #     ('2.max_word_num:', 28)
    text_convas = np.ones([batch_size * FONT_MAX,
                           max_word_num * (vis_size + 2), 3],
    ...
    real_imgs = np.transpose(real_imgs, (0, 2, 3, 1))
    pad_sze = real_imgs.shape
    # print('0.pad_sze:', pad_sze) ('0.pad_sze:', (1, 256, 256, 3))
    middle_pad = np.zeros([pad_sze[2], 2, 3])
    ...
    for i in range(num):
        ...
        # n x c x h x w --> n x h x w x c
        attn = np.transpose(attn, (0, 2, 3, 1))
        # print('6.attn:', attn.shape) # (24, 128, 128, 3)
        num_attn = cap_lens[i]
        # print('7.num_attn:', num_attn) # 28
        thresh = 2./float(num_attn)
        img = real_imgs[i]
        row = []
        row_merge = []
        ...
        for j in range(num_attn):
            ...
            merged = \
                Image.new('RGBA', (vis_size, vis_size), (0, 0, 0, 0))
            mask = Image.new('L', (vis_size, vis_size), (180))  # (210)
            # print('PIL_im:', PIL_im.size())
            # print('PIL_im:', PIL_att.size())
            merged.paste(PIL_im, (0, 0))
            merged.paste(PIL_att, (0, 0), mask)
            merged = np.array(merged)[:, :, :3]
...
def build_super_images3(real_imgs, captions, cap_lens, ixtoword,
                        attn_maps, att_sze, vis_size=256, topK=5):
    ...
    for i in range(num):
        attn = attn_maps[i].cpu().view(1, -1, att_sze, att_sze)
        attn = attn.sum(1) # ('3.attn:', (1, 64, 64))
        # m = nn.Softmax()
        # attn = m(attn)
        # attn =
        # print('3.attn:', attn.size())
        attn = attn.view(-1, 1, att_sze, att_sze)
        # print('4.attn:', attn.size())
        attn = attn.repeat(1, 3, 1, 1).data.numpy()
        # print('5.attn:', attn.shape)
        # n x c x h x w --> n x h x w x c
        attn = np.transpose(attn, (0, 2, 3, 1))
        print('6.attn:', attn.shape)
        print('7.num_attn:', num_attn) # 28
        thresh = 2./float(num_attn)
        # print('8.thresh:', thresh) #('8.thresh:', 0.0625)
        img = real_imgs[i]
        row = []
        row_merge = []
        row_txt = []
        ...
        for j in range(num_attn):
            ...
            # print('mask:', mask)
            one_map = one_map * mask
            # ('one_map.shape:', (64, 64, 3))
            # ('att_sze:', 64)
            # print('upscale=vis_size // att_sze:', vis_size // att_sze) 4
            if (vis_size // att_sze) > 1:
                one_map = \
                    skimage.transform.pyramid_expand(one_map, sigma=20,
                                                     upscale=vis_size // att_sze)
        ...
        # if txt.shape[1] != row.shape[1]:
        #     print('Warnings: txt', txt.shape, 'row', row.shape,
        #           'row_merge_new', len(row_merge_new))
        #     bUpdate = 0
        #     break
        # row = np.concatenate([txt, row_merge], 0)
        img_set.append(row_merge)
    if bUpdate:
        img_set = np.concatenate(img_set, 0)
        img_set = img_set.astype(np.uint8)
        return img_set, sentences
...
def print_para_nums(net, name, only_trainale=False):
    num_params = 0
    ...
    print('[Network %s] Total number of %s parameters : %.6f M' % (name, "trainable" if only_trainale else "", num_params / 1e6))
